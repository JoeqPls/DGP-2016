...
std::vector< Vector3d >
Transformation::
transformPoints( const std::vector< Vector3d > & ps )
{
	std::vector< Vector3d > ps_out = ps;
	for(int i = 0; i < (int) ps.size(); i++)
		ps_out[i] = transformPoint( ps[i] );
	return ps_out;
}
...
std::vector< Vector3d >
Transformation::
transformVectors( const std::vector< Vector3d > & vs )
{
	std::vector< Vector3d > vs_out = vs;
	for(int i = 0; i < (int) vs.size(); i++)
		vs_out[i] = transformVector( vs[i] );
	return vs_out;
}
